...
public class TaskData {
    ...
    public static Set<Task> getTasks(String owner) {
        ...
        String user = "anncarolbob".contains(owner.toLowerCase()) ? owner : null;
        ...
        for (String lineData : selectedList.split("\n")) {
            String[] data = lineData.split(",");
            Arrays.asList(data).replaceAll(String::trim);
            Task.Status status = data.length > 3 ? Task.Status.valueOf(data[3].toUpperCase().replaceAll(" ", "_")) : Task.Status.IN_QUEUE;
